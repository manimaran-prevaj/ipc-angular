...
export class IngredientsHandler {
	...
	public getIngredientsPriceAndCalories(
		...
		addToCartIngredients.forEach((ingredient) => {
			...
			if (isHalfOption) {
				// TODO move hardcoded 888 to constants that reflects v1 data enum [888,999,1000]
				if (halfToggle === false && ingredientQuantity % 2 !== 0 && productOptionData.same_topping_max_count !== 888 ) {
					halfToggle = true;
					logicalQuantity = Math.round(ingredientQuantity / 2);
				} else if (halfToggle === true && ingredientQuantity % 2 !== 0 && productOptionData.same_topping_max_count !== 888 ) {
					halfToggle = false;
